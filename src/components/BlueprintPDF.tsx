...
function buildBlueprintSVG(config: KombiConfig): string {
  ...
  const dimLine = (x1: number, y1: number, x2: number, y2: number, label: string, above = true) => `
    <line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#60a5fa" stroke-width="0.8" marker-start="url(#arr)" marker-end="url(#arr)"/>
    <text x="${(x1 + x2) / 2}" y="${above ? Math.min(y1, y2) - 6 : Math.max(y1, y2) + 14}" text-anchor="middle" fill="#60a5fa" font-size="10" font-family="monospace" font-weight="bold">${label}</text>
  `;
  ...
  <defs>
    ...
    <marker id="arr" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
      <path d="M0,0 L6,3 L0,6" fill="none" stroke="#60a5fa" stroke-width="0.8"/>
    </marker>
  ...
  <text x="${60 + bL - bPillar - 40 - elevL + config.acPosition * elevL}" y="60" text-anchor="middle" fill="#60a5fa" font-size="7" font-family="monospace">A/C</text>
  ...
  ${Array.from({ length: config.solarPanels }).map((_, i) => {
    const px = 60 + bL - bPillar - 40 - elevL + 10 + i * 28;
    return `<rect x="${px}" y="80" width="22" height="${elevH - 4}" fill="#0f172a" stroke="#3b82f6" stroke-width="0.8" rx="2"/>`;
  }).join('')}

  <!-- Dimension lines lateral -->
  ${dimLine(60, 68, 60 + bL, 68, `${(bL / scale).toFixed(1)}m (comprimento total)`)}
  ${dimLine(60 + bL - bPillar - 40 - elevL, 68, 60 + bL - bPillar - 40, 68, `${config.roofLength}m (elevação)`)}
  <line x1="${60 + bL + 10}" y1="${elevH + 80}" x2="${60 + bL + 10}" y2="${elevH + 80 + bH}" stroke="#60a5fa" stroke-width="0.8"/>
  <text x="${60 + bL + 22}" y="${elevH + 80 + bH / 2}" fill="#60a5fa" font-size="9" font-family="monospace" font-weight="bold" dominant-baseline="middle">${(bH / scale).toFixed(1)}m</text>
  <line x1="${60 + bL + 10}" y1="80" x2="${60 + bL + 10}" y2="${elevH + 80}" stroke="#60a5fa" stroke-width="0.8"/>
  <text x="${60 + bL + 22}" y="${40 + elevH / 2}" fill="#60a5fa" font-size="9" font-family="monospace" font-weight="bold" dominant-baseline="middle">${config.roofHeight}m</text>
  ...
  <!-- Dim top view -->
  ${dimLine(60, elevH + bH + 118, 60 + bL, elevH + bH + 118, `${(bL / scale).toFixed(1)}m`)}
  <line x1="${60 + bL + 10}" y1="${elevH + bH + 122}" x2="${60 + bL + 10}" y2="${elevH + bH + 122 + bW}" stroke="#60a5fa" stroke-width="0.8"/>
  <text x="${60 + bL + 22}" y="${elevH + bH + 122 + bW / 2}" fill="#60a5fa" font-size="9" font-family="monospace" font-weight="bold" dominant-baseline="middle">${(bW / scale).toFixed(1)}m</text>
...
export const BlueprintPDF: React.FC<Props> = ({ config }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Abre em nova aba e dispara print (browser converte pra PDF)
